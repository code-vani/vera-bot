...
# Your API key. Read from an environment variable (GROQ_API_KEY) rather than
# hardcoded here, so the key never sits in plaintext in this file.
# Set it in PowerShell before running: $env:GROQ_API_KEY = "gsk_..."
LLM_API_KEY = __import__("os").environ.get("GROQ_API_KEY", "")

# Model to use. openai/gpt-oss-120b is Groq's current production-grade model
# (llama-3.3-70b-versatile still works today but Groq is retiring it 08/16/26).
...
# Which test to run:
#   "all"             -> pass/fail checks only (warmup, auto-reply, intent, hostile) — NO numeric score
#   "full_evaluation" -> pushes the full dataset, scores every generated message with the LLM,
#                         and prints the final AVERAGE SCORE out of 50 (this is the real score)
...
class OpenAIProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req = urlrequest.Request(
            ...
            data=body,
            headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json", "User-Agent": "Mozilla/5.0 (compatible; vera-bot-judge/1.0)"}
        )
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req = urlrequest.Request(
            ...
            headers={"x-api-key": self.api_key, "Content-Type": "application/json",
                     ...
                     "User-Agent": "Mozilla/5.0 (compatible; vera-bot-judge/1.0)"}
        )
        resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
        data = json.loads(resp.read().decode("utf-8"))
...
class BotClient:
    ...
    def push_context(self, scope, cid, version, payload):
        return self._request("POST", "/v1/context", 10, {
            ...
            "payload": payload, "delivered_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S") + "Z"
        })

    def tick(self, triggers):
        return self._request("POST", "/v1/tick", 15, {
            "now": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S") + "Z", "available_triggers": triggers
        })
    ...
    def reply(self, conv_id, merchant_id, message, turn):
        return self._request("POST", "/v1/reply", 15, {
            ...
            "received_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S") + "Z", "turn_number": turn
        })
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
    ...
    def _complete_with_retry(self, prompt: str, max_retries: int = 4) -> str:
        """Call the LLM, retrying with backoff on rate limits (HTTP 429)
        instead of immediately giving up and returning a fake default score."""
        delay = 5
        last_err = None
        for attempt in range(max_retries):
            try:
                ...
                if e.code == 429 and attempt < max_retries - 1:
                    print_warn(f"Rate limited (429) — waiting {delay}s before retry {attempt + 1}/{max_retries - 1}...")
                    time.sleep(delay)
...
class JudgeSimulator:
    ...
    def _auto_reply(self) -> bool:
        ...
        for i in range(1, 5):
            ...
            data, err, _ = self.client.reply(conv_id, mid, auto_msg, i + 1)
    ...
    def _full(self) -> bool:
        ...
        for i in range(0, len(tids), 5):
            ...
            for action in actions:
                ...
                time.sleep(2.5)  # pace requests to stay under Groq free-tier rate limits
